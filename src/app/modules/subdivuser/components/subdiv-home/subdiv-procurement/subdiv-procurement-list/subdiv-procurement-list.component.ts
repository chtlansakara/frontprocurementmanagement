...
@Component({
  selector: 'app-subdiv-procurement-list',
  standalone: false,
  templateUrl: './subdiv-procurement-list.component.html',
...
export class SubdivProcurementListComponent {
 ...
 //table columns (names must match with returned object)
  ...
  loadList(){
    //server call
    this.subdivService.getProcurement()
    .subscribe(res => {
  ...
  //to get status list
  loadStagesList(){
    this.subdivService.getStages().subscribe(res => {
      this.stagesList = res;
    })
  }
...
// for status filter ------------------------------------------------
  ...
  ViewProcurement(row: ProcurementResponseDto){
    this.router.navigateByUrl("/subdivuser/home/procurement/view/"+row.id);
